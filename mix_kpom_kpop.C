...
int main(){
    ...
    for(int k=0;k<nevents;k++){
	    ...
	    for(Int_t i=0;i<mult-1;i++){
            ...
            if(fabs(p1.PseudoRapidity()) > 1.0)   continue;//Eta -1,1
			for(Int_t j=i+1;j<mult;j++){
					...
					if((id[i]==particle1&&id[j]==particle2) || (id[i]==particle2&&id[j]==particle1)){				
                        ...
                        if(fabs(p2.PseudoRapidity()) > 1.0)   continue;//Eta -1,1
                        p1.SetPxPyPzE(px[i],py[i],pz[i],energy[i]);
        ...
        if(hasOmega==true){
			for(int j=0;j<mult;j++){
                //has omega event
                if(id[j]!=particle1 && id[j]!=particle2)	continue;
                //cout << "This particle is omega" << endl;
				TLorentzVector particle;
				particle.SetPxPyPzE(px[j], py[j], pz[j], energy[j]); 
                if(fabs(particle.PseudoRapidity()) > 1.0)   continue;//Eta -1,1
				pid1.push_back(id[j]);
				fpx1.push_back(px[j]);
				fpy1.push_back(py[j]);
				fpz1.push_back(pz[j]);
        ...
        if(hasAntiOmega==true){
			for(int j=0;j<mult;j++){
                //has omega event
                if(id[j]!=particle3 && id[j]!=particle4)	continue;
                //cout << "This particle is omega" << endl;
				TLorentzVector particle;
				particle.SetPxPyPzE(px[j], py[j], pz[j], energy[j]); 
                if(fabs(particle.PseudoRapidity()) > 1.0)   continue;//Eta -1,1
          ...
          //  cout << "mixing particle number of pair 1: "<< pid1.size() <<endl;
            ...
            if(event_pool2==10){
                ...
                for(int m=0;m<pid2.size()-1;m++){
                    ...
                    //in case of kaon rapidity greater than 0.5 or less than -0.5
                    for(int k=m+1;k<pid2.size();k++){   
                        if(pid2[m]==pid2[k])  continue;
                        TLorentzVector p2;
   ...
   //k normalization kpop
	...
	norm_k2->SetName("norm_kpop_k");
    norm_k2->SetTitle("k^{+}#Omega^{+}");
	


    TH1D *kpom_deduction_kpop_pt = new TH1D("kpom_deduction_kpop_pt","k^{+}#Omega^{-}-k^{+}#Omega^{+};#Deltap_{T};counts",40,0,4);
	TH1D *kpom_deduction_kpop_k =  new TH1D("kpom_deduction_kpop_k","k^{+}#Omega^{-}-k^{+}#Omega^{+};k^{*};counts",40, 0.0, 4);
   
    kpom_deduction_kpop_pt->Sumw2();
    kpom_deduction_kpop_k->Sumw2();

    kpom_deduction_kpop_pt->Add(norm_pt1,norm_pt2,1,-1);
    kpom_deduction_kpop_k->Add(norm_k1,norm_k2,1,-1);

    TFile *result_corr = new TFile("kpom_kpop_mix.root","RECREATE");
    same_pt1->Write();
    mix_pt1->Write();
    norm_pt1->Write();
	same_k1->Write();
	...
	mix_k2->Write();
	norm_k2->Write();	
    kpom_deduction_kpop_pt->Write();
    kpom_deduction_kpop_k->Write();
    result_corr->Write();
    return 0;
}
