...
int main(int argc ,char *argv[])
{
    ...
    for (Int_t nevent = 0; nevent < nevents; nevent++)
    {
        ...
        {
            if(id[j]!=3334 && id[j] != -3334)   continue;
            TLorentzVector p2;
...
//    samephi_pair2->Scale(1.0/same_event_pair2);
    ...
    //kstar deduction
    TH1D *deduction_kstar = new TH1D("deduction_kstar","k^{+}#Omega^{-}-k^{+}#bar{#Omega}^{+};k^{*}",40,0,4);
    deduction_kstar->Sumw2();
    deduction_kstar->Add(samek_pair1,samek_pair2,1,-1);
    deduction_kstar->Draw("e");
    //Delta y deduction
    TH1D *deduction_y = new TH1D("deduction_y","k^{+}#Omega^{-}-k^{+}#bar{#Omega}^{+};#Deltay;",80,-4,4);
    deduction_y->Sumw2();
    deduction_y->Add(samey_pair1,samey_pair2,1,-1);
 
    //Delta pt deduction
    TH1D *deduction_pt = new TH1D("deduction_pt","k^{+}#Omega^{-}-k^{+}#bar{#Omega}^{+};#Deltap_{T}",40,0,4);
    deduction_pt->Sumw2();
    deduction_pt->Add(samept_pair1,samept_pair2,1,-1);
    //phi
    TH1D *deduction_phi = new TH1D("deduction_phi","k^{+}#Omega^{-}-k^{+}#bar{#Omega}^{+};#Delta#phi",63,0,6.3);
    deduction_phi->Sumw2();
    deduction_phi->Add(samephi_pair1,samephi_pair2,1,-1);

    TString filename = "deduction_kpom_kpop_";
