...
//void Check_Cf_for_Weijie(const Char_t *inFile = "placeholder.list", const TString JobID = "1234", const int mode = 1)
//  mode 0: AMPT default
//  mode 1: AMPT string-melting or UrQMD (will add UrQMD cendef)
//int Check_Cf_for_Weijie(int fileStart , int fileEnd)
void Check_Cf_for_Weijie(){
...
//  {   
    ...
    // input
    if ((CutEta && Cuty) == true) {cout << "Cannot have both eta and y cuts. Exiting..." << endl; return -1;}
	// int nfile = 0;
	...
	TFile fout("fname_out.root","RECREATE");
    // output
    // char fname_out[200];
    ...
    TChain* chain = new TChain("hadronTree");
    //root file 
    ...
    float imp;
    int pid_vec[maxTrack];
    float px_vec[maxTrack],py_vec[maxTrack],pz_vec[maxTrack];
    int refmult;
    ...
    chain->SetBranchAddress("b", &imp);
    chain->SetBranchAddress("nMultiplicityTree", &refmult);
    chain->SetBranchAddress("id", &pid_vec);
    ...
    for (int i = 0; i < nentries; ++i)
    {
        ...
        //cout << "impact parameter: " << imp << endl;
        // fill track vectors and QA plots
       // int ntrack = pid_vec->size();
       int ntrack = sizeof(pid_vec)/sizeof(pid_vec[0]);
